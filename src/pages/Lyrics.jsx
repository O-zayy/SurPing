...
import { Search, Loader2, Play, Pause, AlertCircle } from 'lucide-react';
...
export default function Lyrics() {
  ...
  return (
    <div className="relative min-h-screen bg-[#121212] font-sans text-white overflow-hidden flex flex-col">
      ...
      {/* Top Search Bar (Absolutely positioned to prevent layout shift) */}
      <div 
        className={`absolute left-0 right-0 z-50 flex justify-center pointer-events-none transition-all duration-[800ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
          selectedSong ? 'top-24 px-5 md:top-28 md:px-8' : 'top-1/2 mt-12 px-6 md:mt-24'
        }`}
      >
        <motion.div 
        ...
        >
          {selectedSong ? (
            <div className="w-full flex items-center bg-black/20 backdrop-blur-3xl saturate-150 border border-white/10 rounded-full shadow-[0_8px_32px_rgba(0,0,0,0.4)] px-5 transition-all outline-none focus-within:bg-black/30 focus-within:border-white/20 focus-within:shadow-[0_0_20px_rgba(255,255,255,0.1)]" style={{ height: 52 }}>
            ...
            <GlassSurface 
              ...
              height={64}
              borderRadius={32}
              className="flex items-center transition-all px-6 py-4 shadow-2xl"
              style={{ padding: 0 }}
            >
              <div className="flex w-full h-full px-5 items-center bg-transparent">
                ...
                <input 
                  className="w-full min-w-0 bg-transparent border-none outline-none text-white placeholder-white/80 font-medium text-lg"
                  type="text" 
                  placeholder="Search for a song or artist..." 
                  value={query}
                  onChange={(e) => setQuery(e.target.value)}
                  ...
                  <div className="flex flex-col w-full bg-black/20 backdrop-blur-3xl saturate-150 border border-white/10 rounded-2xl max-h-[400px] overflow-y-auto no-scrollbar shadow-[0_16px_40px_rgba(0,0,0,0.5)]">
                    {results.map((song) => (
                      <button
                        ...
                        className="w-full text-left flex items-center gap-4 hover:bg-white/10 transition-colors border-b border-white/5 last:border-none px-4 py-3"
                      >
                    ...
                    <div className="flex flex-col w-full h-full max-h-[400px] overflow-y-auto no-scrollbar">
                      {results.map((song) => (
                        <button
                          key={song.id}
                          onClick={() => fetchLyrics(song)}
                          className="w-full text-left flex items-center gap-4 hover:bg-white/10 transition-colors border-b border-white/10 last:border-none px-6 py-4"
                        >
                          <img src={getArtworkUrl(song.image)} alt="Thumbnail" className="w-12 h-12 rounded-md object-cover shadow-md" />
                          <div className="flex flex-col">
                            <span className="text-white font-bold line-clamp-1" dangerouslySetInnerHTML={{ __html: song.title || song.song }}></span>
      ...
      {/* Main Area */}
      <div className="relative z-10 flex-1 flex flex-col w-full h-full pb-8">
...
{/* Error State (Only when no song is selected) */}
          ...
          <div ref={errorContainerRef} className="flex-1 flex flex-col items-center justify-center gap-4 relative w-full">
              ...
              <VariableProximity
                label={error}
                className="text-[#e1e9e5] text-3xl md:text-5xl lg:text-6xl font-bold drop-shadow-md text-center cursor-default tracking-tight"
              radius={120}
              falloff="linear"
            />
          </div>
        ...
        {!selectedSong && !loadingLyrics && !error && (
          <div className="flex-1 flex flex-col items-center justify-center text-center px-6 h-[calc(100vh-200px)] -mt-10">
            <WordsPullUp 
              text="Lyrics Finder" 
              className="text-6xl md:text-8xl lg:text-[120px] font-bold leading-[0.9] tracking-[-0.05em] text-white mb-6 drop-shadow-2xl"
              showAsterisk={true}
            />
            <motion.p 
              ...
              className="text-white/70 text-lg md:text-2xl max-w-3xl mx-auto font-light drop-shadow-md"
            >
        ...
        {!loadingLyrics && selectedSong && (
          <div className="absolute inset-0 pt-40 pb-12 px-6 md:pt-44 md:px-12 lg:px-24 flex items-center justify-center pointer-events-none z-10">
            <div className="w-full h-full max-w-[1400px] flex flex-col md:flex-row items-center justify-between gap-12 lg:gap-24 pointer-events-auto">
              
                {/* LEFT COLUMN: PLAYER */}
              <div className="w-full md:w-1/2 max-w-[500px] flex flex-col justify-center gap-8 h-full py-8">
                ...
                <motion.div 
                  ...
                  className="self-center flex-none rounded-[28px] overflow-hidden shadow-[0_30px_60px_rgba(0,0,0,0.5)] bg-white/5 ring-1 ring-white/10 cursor-pointer group relative"
                  style={{
                    width: "min(430px, 100%, calc(100vh - 360px))",
                    height: "min(430px, 100%, calc(100vh - 360px))"
                  }}
                >
                ...
                <div className="flex flex-col gap-8">
                  ...
                  <div className="flex flex-col gap-2">
                    <h2 className="text-3xl md:text-5xl font-bold text-white line-clamp-1" dangerouslySetInnerHTML={{ __html: selectedSong.title || selectedSong.song }}></h2>
                    <span className="text-xl md:text-2xl text-white/60 font-medium line-clamp-1" dangerouslySetInnerHTML={{ __html: selectedSong.singers }}></span>
                  </div>
              ...
              <div 
                ...
                className="w-full md:w-1/2 h-full overflow-y-auto no-scrollbar scroll-smooth relative"
                style={{ maskImage: 'linear-gradient(to bottom, transparent, black 15%, black 85%, transparent)', WebkitMaskImage: 'linear-gradient(to bottom, transparent, black 15%, black 85%, transparent)' }}
              >
                <div className="w-full flex flex-col items-start gap-8 py-[40vh] pl-4 md:pl-12">
                ...
                {syncedLyrics.length > 0 ? (
                    ...
                    return (
                      <p 
                        ...
                        className={`transition-all duration-[600ms] ease-out cursor-pointer origin-left text-left w-full ${
                          isActive 
                            ? 'text-4xl md:text-[56px] lg:text-[64px] font-bold text-white leading-tight scale-100 opacity-100 drop-shadow-[0_4px_10px_rgba(0,0,0,0.5)]' 
                            : 'text-3xl md:text-[44px] lg:text-[52px] font-bold text-white/40 leading-tight scale-95 opacity-50 hover:text-white/70 hover:opacity-100'
                        }`}
                        onClick={() => {
                          if (audioRef.current) {
                  ...
                  plainLyricLines.map((line, i) => (
                    <p 
                      key={i}
                      className="text-3xl md:text-[44px] lg:text-[52px] font-bold text-white/80 leading-tight text-left w-full"
                    >
                      {line}
                    </p>
                  ))
                    ...
                    <div className="flex flex-col items-center justify-center gap-8 relative w-full pt-[10vh] pr-4">
                      ...
                      <div ref={errorContainerRef} className="w-full flex justify-center pb-20">
                        <VariableProximity
                          label={error}
                          className="text-[#e1e9e5] text-5xl md:text-6xl lg:text-7xl font-bold drop-shadow-lg text-left w-full cursor-default tracking-tight leading-[1.1] opacity-90"
